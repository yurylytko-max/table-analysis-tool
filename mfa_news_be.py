...
PAGE_URL = f"{BASE_URL}/be/press/news_mfa/"
API_URL = f"{BASE_URL}/be/press/news_mfa/json/?get_more_news"
OUTPUT_FILE = Path("mfa_news_be.csv")
MAX_PAGES = 5000
RETRIES = 3
LIST_DELAY_MIN = 0.25
LIST_DELAY_MAX = 0.45
...
def extract_content(html: str) -> str:
    match = re.search(
        r'<span class="date _big">.*?</span>(.*?)(?:<p>\s*<a href="/print/|<div class="content-page__social")',
        html,
        flags=re.S | re.I,
    )
    if not match:
...
def build_or_resume_index(session: requests.Session, rows: List[Dict[str, str]], gtoken: str) -> List[Dict[str, str]]:
    ...
    for page in range(1, MAX_PAGES + 1):
        ...
        if no_progress_pages >= NO_PROGRESS_PAGE_LIMIT:
            print(f"stop: saw {NO_PROGRESS_PAGE_LIMIT} no-progress pages in a row")
            stop_page = page
            break
